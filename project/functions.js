var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	*/
	...
	return [
		[1, "先攻", "先发制人。怪物首先攻击。"],
		[2, "魔攻", "魔法攻击无法被护甲抵挡。怪物无视勇士的防御。"],
		[3, "坚固", "很硬。勇士每回合最多只能对怪物造成1点伤害。"],
		[4, "2连击", "怪物每回合攻击2次"],
		[5, "3连击", "怪物每回合攻击3次"],
		[6, function (enemy) { return (enemy.n || 4) + "连击"; }, function (enemy) { return "怪物每回合攻击" + (enemy.n || 4) + "次"; }],
		[7, function (enemy) { return (enemy.value || 0) + "倍破甲"; }, function (enemy) { return "侵蚀对手的护甲。首回合怪物攻击时，附加勇士防御的" + Math.floor(100 * enemy.value || 0) + "%作为伤害"; }],
		[8, function (enemy) { return (enemy.value || 0) + "倍反伤"; }, function (enemy) { return "来犯之敌，自讨苦吃。战斗时，勇士攻击的" + Math.floor(100 * enemy.value || 0) + "%也会同时伤害到自身"; }],
		...
		[14, "霜寒", function (enemy) { return "怪物对敌人施加霜寒诅咒。战斗后，你获得" + enemy.n + "层霜寒状态，每层使你普通攻击造成的伤害降低1%，加法叠加。"; }],
		...
		[99, "闪避", function (enemy) { return "灵巧的身法能够躲闪攻击。受到的普通攻击伤害降低" + (enemy.defValue || 0) + "%。"; }],
		[100, "穿刺", function (enemy) { return "攻击能够穿透一部分防御。无视对手" + (enemy.x || 0) + "%的防御力。"; }],
		[101, "夹爆", "某个著名红海技能的上位版本。经过两只相同的怪物中间，勇士生命值变成一。"],
		[102, "上位威压", function (enemy) { return "上位者的气质震慑对手。每比对手高出一级，便在先前基础上进一步削弱对手" + Math.floor(enemy.n) + "%的攻防，当前对方比你高" + Math.max(0, (enemy.value || 0) - core.status.hero.lv) + "级"; }],
		[103, "强击", function (enemy) { return "一次强力的攻击。怪物首回合获得" + (enemy.atkValue || 0) + "倍攻击力。"; }],
		[104, "斩杀", function (enemy) { return "一旦对手势弱，攻击就会更加致命。战斗中，若勇士生命值低于" + (enemy.range || 0) + "%，则怪物攻击力提升" + (enemy.n || 0) + "%。"; }],
		[105, "重甲", function (enemy) { return "无视对手" + (enemy.defValue || 0) + "%的攻击力。"; }],
		[106, "寒霜之足", function (enemy) { return "危险的寒冰咒印。怪物前5回合每回合额外造成" + enemy.damage + "点伤害，然后冰封对方10回合。"; }],
		[107, "冰封禁制", function (enemy) { return "将敌人锁在冰块之中。怪物首先冰封对方3回合，每回合额外造成" + enemy.damage + "点伤害。"; }],
		[108, "霜寒爆发", "令敌人体内的霜寒尽数爆发。战斗开始时，立即清除勇士的所有霜寒状态，每驱除1层就会使勇士冰封1回合。"],
		[109, "寒冰护甲", "冷气贯穿全身。每受到1回合攻击就会使勇士获得1层霜寒状态，战斗后生效。"],
		[110, "光明", function (enemy) { return "战斗后，勇士获得" + enemy.n + "层光明状态，每层使得勇士受到和造成的伤害均提升1%，线性叠加。会与黑暗状态相抵消。"; }],
		[111, "黑暗", function (enemy) { return "战斗后，勇士获得" + enemy.n + "层黑暗状态，每层使得勇士受到和造成的伤害均减少1%，线性叠加。会与光明状态相抵消。"; }],
		[112, "迅捷", "迅捷的反应令对手猝不及防。怪物先攻10次。"],
		[113, "迟钝", "迟钝的反应令自己猝不及防。勇士先攻10次。"],
		...
		[118, "炫目之光", "持续散发出神圣的光芒之力。每回合损耗自身2%的最大生命值，令对手无法攻击。"],
